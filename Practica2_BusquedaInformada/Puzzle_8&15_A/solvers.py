...
class AStarPuzzleSolver:
    ...
    def get_h(self, state):
        if self.h_type == "Fichas fuera": return PuzzleModel.h_misplaced(state, self.goal)
        if self.h_type == "Manhattan": return PuzzleModel.h_manhattan(state, self.goal)
        return PuzzleModel.h_custom(state, self.goal)
    ...
    def solve(self):
        ...
        frontier = [(self.get_h(self.start), 0, start_tuple, [])]
        ...
        while frontier:
            ...
            if np.array_equal(curr_state, self.goal):
                end_time = time.perf_counter()
                _, peak = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                return True, path + [curr_tuple], end_time - start_time, peak / 1024

            nodes_expanded += 1
            if nodes_expanded > 5000: break

            r, c = np.where(curr_state == 0)
            r, c = r[0], c[0]
            ...
            for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if 0 <= nr < self.size and 0 <= nc < self.size:
                    ...
                    if new_tuple not in visited or new_g < visited[new_tuple]:
                        ...
                        f_new = new_g + self.get_h(new_state)
