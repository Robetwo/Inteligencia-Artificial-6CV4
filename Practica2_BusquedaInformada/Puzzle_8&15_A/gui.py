...
class PuzzleApp(ctk.CTk):
    ...
    def setup_styles(self):
        ...
        style.configure("Treeview", background="#2b2b2b", foreground="white", fieldbackground="#2b2b2b", font=("Segoe UI", 12))
        style.configure("Treeview.Heading", background="#1f1f1f", foreground="white", font=("Segoe UI", 14, "bold"))
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(title_frame, text="Puzzle", text_color="yellow", font=ctk.CTkFont(size=28, weight="bold")).grid(row=0, column=1, padx=5)

        ctk.CTkLabel(self.sidebar, text="Configuración", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=10)

        ctk.CTkLabel(self.sidebar, text="Tamaño:", font=("bold", 14)).pack(pady=10)
        self.puzzle_type = ctk.CTkSegmentedButton(self.sidebar, values=["3x3 (8)", "4x4 (15)"], font=ctk.CTkFont(size=14, weight="bold"), command=self.change_size)
        self.puzzle_type.set("3x3 (8)")
        self.puzzle_type.pack(padx=20, pady=10)

        self.btn_change_img = ctk.CTkButton(self.sidebar, text="CAMBIAR FOTO", height=45, font=ctk.CTkFont(size=14, weight="bold"), fg_color="#34495e", command=self.cycle_image)
        self.btn_change_img.pack(pady=2, padx=20)

        self.btn_random = ctk.CTkButton(self.sidebar, text="MEZCLAR", height=45, font=ctk.CTkFont(size=14, weight="bold"), fg_color="#CF530A", command=self.generate_puzzle)
        ...
        ctk.CTkOptionMenu(self.sidebar, values=["Fichas fuera", "Manhattan", "Weighted A*"], variable=self.h_var).pack(pady=10)

        self.btn_solve = ctk.CTkButton(self.sidebar, text="RESOLVER", height=50, font=ctk.CTkFont(size=14, weight="bold"), fg_color="#149900", command=self.run_ai)
    ...
    def generate_puzzle(self):
        ...
        # --- CORRECCIÓN LÓGICA: MEZCLA GARANTIZADA SOLUBLE ---
        state = np.arange(1, self.size ** 2 + 1).reshape(self.size, self.size)
        state[-1, -1] = 0
        r, c = self.size - 1, self.size - 1

        # Mezclamos haciendo movimientos reales desde la meta
